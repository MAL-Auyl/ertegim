...
function buildMessages({ transcript, questionKk, criterion, alternatives = [], expectedForms = [] }) {
  ...
  // One line per alternative, labelled with the tag it ACTUALLY carries.
  // The old fixed «kk: … / ru: …» pair lied under STT_LANGS=auto,ru: when
  // Whisper's auto pass detects ru, both candidates are ru, and the prompt
  // presented a Russian string as the Kazakh recognition — twice, as if two
  // independent opinions agreed. De-duplicated by exact text for the same
  // reason: identical strings are one opinion, not two.
  const seen = new Set();
  const lines = [];
  for (const a of alts) {
    ...
    seen.add(text);
    const lang = a && typeof a === "object" && a.lang ? a.lang : "stt";
    lines.push(`распознавание ${lang}: "${text}"`);
  }
  // No usable alternatives (e.g. a legacy caller passing none): fall back to
  // the picked transcript, which is what the old single-recognition prompt used.
  if (!lines.length) lines.push(`распознавание stt: "${String(transcript ?? "")}"`);
  const recognitions = lines.join("; ");
