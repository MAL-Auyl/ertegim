...
как казахский и как русский **параллельно**, а `lib/stt-pick.js` выбирает тот
...
счёта — `trackCount`, вписать ожидаемый ответ напротив каждого файла и нажать
«Run turbo» / «Run v3». Таблица покажет оба транскрипта, выбранный, score,
confidence и совпадение с ожидаемым; внизу — точность и среднее время на файл
для каждой модели.
